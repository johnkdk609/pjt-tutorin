<template>
  <div>
    <span v-for="counsel in proceedCounselList">
      <div class="button" @click="open(counsel.id)" :key="counsel.id" v-if="loginStore.loginUser.id == counsel.mentorId">
        <div class="mentor">
        ...
        <div class="content">
          <h1>{{ counsel.title }}</h1>
          <h5>답변 마감 시간 : {{ counsel.acceptTime }}</h5>
        </div>
        <div class="status">
